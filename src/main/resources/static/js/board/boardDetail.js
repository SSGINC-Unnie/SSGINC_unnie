    ...
    const fetchComments = async () => {
        try {
            ...
            if (response.ok) {
                ...
                if (commentCountSpan) commentCountSpan.textContent = rootCommentCount;
    ...
    if (deleteButton) {
        deleteButton.addEventListener('click', async () => {
            if (confirm('정말로 이 게시글을 삭제하시겠습니까?')) {
                try {
                    const response = await fetch(`/api/community/board/${boardId}`, { method: 'PATCH' });
                    const result = await response.json();
                    if (response.ok) {
                        alert('게시글이 삭제되었습니다.');
    ...
    if (reportForm) {
        reportForm.addEventListener('submit', async (e) => {
            ...
            const reportData = {
                ...
                reportReasonDetailed: document.getElementById('report-reason-detailed').value
            };
            try {

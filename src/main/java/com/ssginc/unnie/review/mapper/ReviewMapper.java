...
public interface ReviewMapper {
     ...
     * ReviewCreateRequest에는 reviewMemberId, reviewReceiptId, reviewImage, reviewRate,
     * reviewContent, reviewEmotionalScore, reviewStatus, 그리고 (후에) keywordIds가 포함되어 있다.
     */
    int insertReview(ReviewCreateRequest review);

    /**
     * 리뷰-키워드 연결 정보 등록: review_keyword 테이블에 (keyword_id, review_id)를 삽입한다.
     * ReviewCreateRequest의 keywordIds 리스트와 reviewId를 활용한다.
     */
    int insertReviewKeywords(ReviewCreateRequest review);
     ...
     * 이 메서드는 resultMap의 <collection>에서 호출된다.
     */
